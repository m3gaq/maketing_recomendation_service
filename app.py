...
def main():
    ...
    if 'Оценка костов будущей маркетинговой кампании' in selected_instruments:
        ...
        st.write('''Перед запуском кампании мы хотим понять, какой оффер нам нужно предлагать клиентам и сколько дней предлагать, чтоб активировать этот оффер, на какой день предоагать оффер.''')
        st.write('''
